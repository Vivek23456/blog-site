...
/// Frontmatter structure for each post
...
/// Find and load all frontmatter TOML files from ./posts
        ...
        match frontmatter {
            Ok(entry) => {
                if entry.path().is_file() {
                    let fm_content = fs::read_to_string(entry.path())?;
...
pub async fn index(templates: web::Data<tera::Tera>) -> impl Responder {
    ...
    // Sort posts by order (descending)
